...
def test_formal_runtime_is_codex_gpt_56(tmp_path) -> None:
    ...
    assert runtime_value.agent_import == "taskfoundry.portable_codex_agent:PortableCodex"
    config = build_job_config(spec(tmp_path, model_name=runtime_value.model_name), runtime_value)
    kwargs = config["agents"][0]["kwargs"]
    assert kwargs["portable_code_mode_host_path"] == str(host)
    assert config["agents"][0]["env"]["OPENAI_BASE_URL"] == "${STRONG_BASE_URL}"
...
def test_codex_runtime_rejects_invalid_formal_configuration(runtime_value, message) -> None:
...
def test_context_must_exist(tmp_path) -> None:
    ...
    with pytest.raises(ContractError, match="context"):
        build_job_config(spec(tmp_path, context_paths=(str(missing),)), runtime(tmp_path))
