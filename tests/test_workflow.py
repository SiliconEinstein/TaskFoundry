...
def audit_bound(flow, tmp_path, index, *, mode="blind", score=0.4):
    ...
    leakage.write_text('{"passed":true}')
...
def test_bound_health_can_be_accepted_after_runtime_environment(tmp_path) -> None:
    ...
    flow.start_blind_validation(Actor.TEACHER, "blind")
    flow._record_attempt(Actor.REVIEWER, attempt(1), "audit-1")
    flow.bind_runtime_environment(Actor.LABWRIGHT, receipt(tmp_path), "runtime-environment")
    ...
    flow._record_attempt(Actor.REVIEWER, attempt(2), "audit-2")
    flow._record_attempt(Actor.REVIEWER, attempt(3), "audit-3")
    completed = flow._record_attempt(
        Actor.REVIEWER,
...
def test_failed_hints_can_start_evidence_backed_revision(tmp_path) -> None:
    flow = ready_workflow(tmp_path)
    for index in range(1, 4):
        flow._record_attempt(Actor.REVIEWER, attempt(index), f"audit-{index}")
    flow._record_attempt(Actor.REVIEWER, attempt(1, mode="hint", score=0.7), "hint-audit")
    reason = tmp_path / "solvability-failure.json"
    reason.write_text('{"reason":"hint did not pass"}')
...
def test_revision_can_refresh_stable_environment_before_freeze(tmp_path) -> None:
    flow = ready_workflow(tmp_path)
    for index in range(1, 4):
        flow._record_attempt(Actor.REVIEWER, attempt(index), f"audit-{index}")
    flow._record_attempt(Actor.REVIEWER, attempt(1, mode="hint", score=0.7), "hint-audit")
